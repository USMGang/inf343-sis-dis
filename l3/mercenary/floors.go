...
func (s *FloorsServers) floor1(){
	...
	request_results := f.Floor1ResultsRequest{Id: int32(id), SelectedWeapon: int32(choice), RandNumber: int32(rand.Intn(101))}
	response_results, err := s.Director.Floor1Results(context.Background(), &request_results)
...
func (s *FloorsServers) floor2(){
	...
	path_request := f.Floor2PathRequest{ Id: int32(id), SelectedPath: int32(choice) }
...
func (s *FloorsServers) floor3(){
	...
	for i := 1; i<=5; i++ {
	    ...
	    tries_request := f.Floor3Try{ Id: int32(id), NTries: int32(i), NGoodTries: int32(n_good_tries), RandNumber: int32(rand_number) }
	...
	floor3_request := f.Floor3ResultsRequest{ Id: int32(id), NGoodTries: int32(n_good_tries) }
